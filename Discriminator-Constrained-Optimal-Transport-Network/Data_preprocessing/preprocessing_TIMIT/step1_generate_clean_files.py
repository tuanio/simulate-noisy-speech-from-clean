...
parser.add_argument('--timit-path', default='/data/tuanio/data/timit/data')
parser.add_argument('--target-root', default='/data/tuanio/data/noise/TIMIT_codec2')
parser.add_argument('--sample-rate', default=8000, type=int)
args = parser.parse_args()

def load_and_resample(file, new_sample_rate):
    wav, sr = torchaudio.load(file)
    if sr != new_sample_rate:
        wav = torchaudio.function.resample(wav, sr, new_sample_rate)
    return wav, new_sample_rate
...
def write_wav(list_file, write_path, outfile_name):
    ...
    for line in infile:
        src_path = line.strip()
        wav, sr = load_and_resample(src_path, args.sample_rate)
        ...
        tgt_path = tgt_path.replace('.WAV.wav', '.wav')
        # sph.write_wav(tgt_path)
        print('writing:', tgt_path)
        torchaudio.save(tgt_path, wav, sr)
...
for i in range(1, 9):
    ...
    for s in speakers[0:6]:
        train_list += [entry.path for entry in os.scandir(os.path.join(r, s)) if (entry.name.endswith('.WAV.wav') and not entry.name.startswith('SA'))]   # SA sentances are excluded
...
test_list = [entry.path for folder in test_speaker_folders for entry in os.scandir(folder) if
             (entry.name.endswith('.WAV.wav') and not entry.name.startswith('SA'))]
