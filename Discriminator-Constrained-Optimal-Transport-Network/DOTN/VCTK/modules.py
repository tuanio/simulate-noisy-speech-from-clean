...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.blstm = nn.LSTM(
            257, 1024, dropout=0.0, num_layers=2, bidirectional=True, batch_first=True
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(  # input shape (batch_size, 1, 64, 257)
            ...
            nn.MaxPool2d(
                kernel_size=2
            ),  # choose max value in 2x2 area, output shape (batch_size, 16, 32, 128)
        )
    ...
    def forward(self, x):
        ...
        x = x.view(
            x.size(0), -1
        )  # flatten the output of conv2 to (batch_size, 32 * 16 * 64)
        x = self.out1(x)
        output = self.out2(x)
        return output
