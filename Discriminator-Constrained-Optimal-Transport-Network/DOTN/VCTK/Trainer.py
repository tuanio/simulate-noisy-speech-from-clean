...
class Trainer:
    ...
    def _val_epoch(self):
        ...
        with tqdm(
            total=len(self.loader["val"]),
            desc=f"Validate epoch: {self.epoch}/{self.epochs - 1}",
            unit="step",
        ) as t:
            for X, y in self.loader["val"]:
                ...
                loss = F.mse_loss(
                    y_pred.reshape((y.shape[0], y.shape[1], y.shape[2])),
                    y / self.args.amplify,
                    reduction="sum",
                )
            ...
            self.val_loss /= self.args.batch_size * len(
                self.loader["val"]
            )  # average batch loss
            t.set_postfix(
                {"Validate loss": "{:.3f}".format(self.val_loss)}
            )  # display loss
    ...
    def train(self):
        while self.epoch < self.epochs:
            ...
            self.D.train()
            self.G.train()
            OT_scheduler = ReduceLROnPlateau(
                self.optimizer_G_OT, mode="min", factor=0.1, patience=4
            )

            with tqdm(
                total=len(self.loader["train"]),
                desc=f"Train epoch: {self.epoch}/{self.epochs - 1}",
                unit="step",
            ) as t:
                for batch_idx, (X_s, y_s, X_t) in enumerate(self.loader["train"]):
                    ...
                    if batch_idx % self.period_D == 0:
                        ...
                        loss_d = -torch.mean(self.D(y_s)) + torch.mean(
                            self.D(self.G(X_t).detach())
                        )
                        loss_d.backward()
                        ...
                        batch_D_loss = str(
                            np.around(loss_d.item(), decimals=3)
                        )  # for display
                    ...
                    if batch_idx % self.period_G == 0:
                        ...
                        batch_G_loss = str(
                            np.around(loss_g.item(), decimals=3)
                        )  # for display
                    ...
                    if batch_idx % self.period_S == 0:
                        ...
                        loss_s = self.criterion(
                            self.G(X_s) / self.args.amplify, y_s / self.args.amplify
                        )
                    ...
                    t.set_postfix(
                        {
                            ...
                            "Epoch-OT-Loss": "{:.3f}".format(self.epoch_OT_loss),
                        },
                        refresh=True,
                    )
                    t.update(1)
    ...
    def test(self):
        ...
        for test_file in progress_bar:
            ...
            noise_type = test_file.split("/")[-3]
            snr = test_file.split("/")[-2]
            ...
            y, sr = librosa.load(
                os.path.join(self.args.WAV_test_clean_path, filename), sr=16000
            )
            ...
            # model enhancement
            y_spec_pred = (
                self.G(self.args.amplify * X_spec).detach().cpu().numpy().squeeze(0)
                / self.args.amplify
            )
            ...
            audio_file = os.path.join(out_path, "_".join(test_file.split("/")[-3:]))
            audiowrite(audio_file, 16000, np.int16(y_pred * maxv))

            # compute scores
            score_PESQ, score_STOI = cal_score(
                np.int16(y * maxv), np.int16(y_pred * maxv)
            )
