...
def load_checkoutpoint(
    D, G, optimizer_S, optimizer_D, optimizer_G, optimizer_G_OT, model_path
):
...
def Load_model(args, D, G, checkpoint_path):
    ...
    if args.resume:
        ...
        (
            ...
            D, G, optimizer_S, optimizer_D, optimizer_G, optimizer_G_OT, checkpoint_path
        )
    elif args.retrain:
        ...
        (
            ...
            D, G, optimizer_S, optimizer_D, optimizer_G, optimizer_G_OT, args.model_path
        )
...
def Load_data(args):
    # split data
    train_noisy_paths, val_noisy_paths = train_test_split(
        get_filepaths(args.train_noisy_path, ftype=".pt"),
        test_size=0.1,
        random_state=999,
    )
    train_target_noisy_paths, val_target_noisy_paths = train_test_split(
        get_filepaths(args.target_noisy_path, ftype=".pt"),
        test_size=0.1,
        random_state=1000,
    )
    ...
    train_dataset = CustomDataset(
        ...
        train_noisy=train_noisy_paths,
        target_noisy=train_target_noisy_paths,
    )
    val_dataset = CustomDataset(
        clean_path=args.target_clean_path, train_noisy=val_target_noisy_paths
    )
...
class CustomDataset(Dataset):
    def __init__(self, clean_path, train_noisy, target_noisy=None, amplify=1):
        ...
        self.train_clean = [
            os.path.join(clean_path, f.split("/")[-2], f.split("/")[-1])
            for f in train_noisy
        ]  # find clean correspondance
