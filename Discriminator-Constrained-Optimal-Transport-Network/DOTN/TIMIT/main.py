...
# data location
data_path = "/work/TIMIT_DA_1target_BabyCry/"
pt_data_path = "/work/TIMIT_DA_1target_BabyCry_pt/"
...
def get_args():
    ...
    parser.add_argument(
        "--train_clean_path",
        type=str,
        default=os.path.join(pt_data_path, "train/clean"),
    )
    parser.add_argument(
        "--train_noisy_path",
        type=str,
        default=os.path.join(pt_data_path, "train/noisy"),
    )
    ...
    parser.add_argument(
        "--target_clean_path",
        type=str,
        default=os.path.join(pt_data_path, "test/clean"),
    )
    parser.add_argument(
        "--target_noisy_path", type=str, default=os.path.join(pt_data_path, "test")
    )

    # for final WAV scores: PESQ, STOI
    parser.add_argument(
        "--WAV_test_clean_path", type=str, default=os.path.join(data_path, "test/clean")
    )
    parser.add_argument(
        "--WAV_test_noisy_path", type=str, default=os.path.join(data_path, "test/noisy")
    )
    parser.add_argument(
        "--WAV_enhanced_path", type=str, default="./enhanced"
    )  # save enhanced audio
    ...
    parser.add_argument("--task", type=str, default="Babycry")
    parser.add_argument("--resume", action="store_true")
    parser.add_argument("--retrain", action="store_true")
    parser.add_argument("--re_epochs", type=int, default=300)
    parser.add_argument(
        "--period_S", type=str, default=2
    )  # source domain memory training
    parser.add_argument("--period_D", type=str, default=6)
    parser.add_argument("--period_G", type=str, default=13)
    parser.add_argument("--amplify", type=str, default=1)
    parser.add_argument(
        "--pretrained_model", type=str, default=None
    )  # provide pretrained SE model G if exists
    parser.add_argument(
        "--model_path", type=str, default=None
    )  # provide an existing model for testing
    parser.add_argument(
        "--save_path", type=str, default="./saved_model/"
    )  # saved model path
...
def get_path(args):
    checkpoint_path = os.path.join(
        args.save_path,
        f"{args.model}_epochs{args.epochs}"
    ...
    score_path = (
        f"./results/{args.model}_epochs{args.epochs}"
        f"_{args.optim}_{args.loss_fn}_batch{args.batch_size}_"
...
if __name__ == "__main__":
    ...
    (
        ...
        criterion,
        device,
    ) = Load_model(args, discriminator, generator, checkpoint_path)
