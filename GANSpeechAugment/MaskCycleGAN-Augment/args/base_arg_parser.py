...
class BaseArgParser(object):
    ...
    def __init__(self):
        ...
        self.parser.add_argument(
            ...
            help="Directory for results including ckpts.",
        )
        ...
        self.parser.add_argument(
            "--use_res", action="store_true", help="Generate residual information"
        )
        ...
        self.parser.add_argument(
            ...
            type=str,
            default=["clean", "noisy"],
            help="class IDS of the two domains.",
        )
        self.parser.add_argument(
            "--spec_power",
            dest="spec_power",
            type=float,
            default=1.0,
            help="Number to raise spectrogram by.",
        )
        self.parser.add_argument(
            "--energy",
            dest="energy",
            type=float,
            default=1.0,
            help="to modify the energy/amplitude of the audio-signals",
        )
        self.parser.add_argument(
            "--dataroot",
            dest="dataroot",
            type=str,
            default=DATAROOT_DEFAULT,
        ...
        self.parser.add_argument(
            ...
            type=str,
            default="use_default",
            help="Split to use for data set. If data has not been split into sets pass None.",
        )
        self.parser.add_argument(
            "--data_load_order",
            dest="data_load_order",
            default="use_default",
            type=str,
            choices=["aligned", "unaligned"],
            help="Load Data as aligned or unaligned. For test phase it is unaligned aligned by default and for train it is unaligned by default.",
        )
        self.parser.add_argument(
            "--load_size", type=int, default=128, help="scale images to this size"
        )
        self.parser.add_argument(
            "--preprocess",
            type=str,
            default="resize",
            help="scaling and cropping of images at load time [resize_and_crop | crop | scale_width | scale_width_and_crop | none | passcodec]",
        )
        self.parser.add_argument(
            "--no_flip",
            action="store_true",
    ...
    def parse_args(self):
        ...
        with open(
            os.path.join(args.save_dir, args.name, f"{prefix}_args.json"), "w"
        ) as fh:
        ...
        if len(args.gpu_ids) > 0 and torch.cuda.is_available():
            ...
            args.gpu_ids = ["cuda" + ":" + str(i) for i in range(len(args.gpu_ids))]
            args.device = "cuda"
        else:
            args.device = "cpu"

        # Ensure consistency of load_epoch and start_epoch arguments with each other and defaults.
        if not args.isTrain or (
            hasattr(args, "continue_train") and args.continue_train
        ):
