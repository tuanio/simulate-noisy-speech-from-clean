...
def validate(name, epochs, data_cache, results_dir):
    ...
    for epoch in epochs:
        run(
            f"python -W ignore::UserWarning -m mask_cyclegan_vc.test --name {name} --split val --save_dir {results_dir} --gpu_ids 0 --speaker_A_id clean --speaker_B_id noisy --load_epoch {epoch} --ckpt_dir /content/drive/MyDrive/APSIPA/Results/{name}/ckpts --model_name generator_A2B"
        )
        avg_lsd, std_lsd = lsd(
            os.path.join(data_cache, "noisy", "val"),
            os.path.join(results_dir, name, "audios", "fake_B"),
            use_gender=False,
        )
...
def apsipa_exp(
    ...
    csv_path,
    sources,
    data_cache="/content/MaskCycleGAN-Augment/data_cache",
    results_dir="/content/MaskCycleGAN-Augment/results",
    ...
    for name, source in zip(names, sources):
        ...
        for metric in ["min_lsd_epoch", "min_mssl_epoch"]:
            epoch = info[metric]
            run(
                f"python -W ignore::UserWarning -m mask_cyclegan_vc.test --name {name} --split test --save_dir {results_dir} --gpu_ids 0 --speaker_A_id clean --speaker_B_id noisy --load_epoch {epoch} --ckpt_dir /content/drive/MyDrive/APSIPA/Results/{name}/ckpts --model_name generator_A2B"
            )
        ...
        info[
            "comment"
        ] = f"MaskCycleGAN trained for 150 epochs, WITHOUT phase on {source} dataset."
        log(csv_path, info)
