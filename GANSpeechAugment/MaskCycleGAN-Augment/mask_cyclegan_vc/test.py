...
class MaskCycleGANVCTesting(object):
    ...
    def __init__(self, args):
        ...
        if hasattr(args, "eval") and args.eval:
            ...
            self.eval = False
            self.converted_audio_dir = os.path.join(
                args.save_dir, args.name, "converted_audio"
            )
    ...
    def test(self):
        ...
        ds_len = (
            self.dataset.get_clean_len()
            if self.model_name == "generator_A2B"
            else self.dataset.get_noisy_len()
        )
        idx = 0
        datas = []
        ...
        while idx < ds_len:
            ...
            self.save_audio(
                ...
                visuals_list,
                img_path,
                label="fake_B" if self.model_name == "generator_A2B" else "fake_A",
            )
            idx += 1
