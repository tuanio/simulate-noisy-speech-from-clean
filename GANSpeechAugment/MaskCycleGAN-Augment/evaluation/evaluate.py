...
DATA_DIRECTORY_DEFAULT = (
    "/content/MaskCycleGAN-VC/evaluation/temp_cache/converted_audio"
)
...
def main():
    ...
    argument_parser.add_argument(
        "--speaker_A_id", type=str, default="real", help="Speaker A ID"
    )
    argument_parser.add_argument(
        "--speaker_B_id", type=str, default="generated", help="Speaker B ID"
    )
    ...
    for file in os.listdir(os.path.join(args.data_directory, args.speaker_A_id)):
        norm_tot += norm_and_LSD(
            os.path.join(args.data_directory, args.speaker_A_id, file),
            os.path.join(args.data_directory, args.speaker_B_id, file),
        )
        count += 1
