...
def main(args):
    ...
    source_id, source_path_end = (
        (args.speaker_A_id, "src.wav")
        if args.model_name == "generator_A2B"
        else (args.speaker_B_id, "A.wav")
    )
    target_id, target_path_end = (
        (args.speaker_A_id, "src.wav")
        if args.model_name == "generator_B2A"
        else (args.speaker_B_id, "A.wav")
    )
        ...
        for speaker_id in [source_id, target_id]:
            ...
            os.remove(
                os.path.join(
                    ...
                    "agg",
                    speaker_id,
                    f"{speaker_id}_normalized.pickle",
                )
            )
    ...
    for source_file in os.listdir(os.path.join(args.data_directory, source_id)):
        ...
        shutil.copyfile(
            os.path.join(args.data_directory, source_id, source_file),
            os.path.join(source_orig_data_path, source_file),
        )
        shutil.copyfile(
            os.path.join(args.data_directory, target_id, target_file),
            os.path.join(target_orig_data_path, target_file),
        )

        # Preprocess Data

        for speaker_id in [source_id, target_id]:
            stat = preprocess_dataset(
                data_path=os.path.join(args.eval_cache, "orig", speaker_id),
                speaker_id=speaker_id,
                cache_folder=os.path.join(args.eval_cache, "processed"),
                eval=True,
            )
            if stat:
                os.remove(
                    os.path.join(
                        ...
                        "processed",
                        speaker_id,
                        f"{speaker_id}_norm_stat.npz",
                    )
                )  # Removing individual stats
                shutil.copyfile(
                    os.path.join(
                        args.eval_cache,
                        "agg",
        ...
        # Run inference
        args.eval = True
        args.eval_save_dir = os.path.join(
            args.eval_cache, "converted_audio", "generated"
        )
        ...
        sf.write(
            os.path.join(
                ...
                "converted_audio",
                "real",
                source_file[: -(len(source_path_end) + 1)] + ".wav",
            ),
            real,
            args.sample_rate,
            "PCM_16",
