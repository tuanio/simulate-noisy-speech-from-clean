...
DATA_DIRECTORY_DEFAULT = (
    "/content/drive/MyDrive/NTU - Speech Augmentation/Parallel_speech_data"
)
SPEAKER_IDS_DEFAULT = ["clean", "noisy"]
PREPROCESSED_DATA_DIRECTORY_DEFAULT = (
    "/content/MaskCycleGAN-VC/data_preprocessed/training"
)
...
def normalize_mel(wavspath, eval=False):
    ...
    for wavpath in tqdm(wav_files, desc="Preprocess wav to mel"):
        ...
        if (
            spec.shape[-1] >= 64
        ):  # training sample consists of 64 randomly cropped frames
    ...
    for mel in mel_list:
        assert (
            mel.shape[-1] >= 64
        ), f"Mel spectogram length must be greater than 64 frames, but was {mel.shape[-1]}"
...
def preprocess_dataset(data_path, speaker_id, cache_folder="./cache/", eval=False):
    ...
    if info[
        "success"
    ]:  # If it is not in eval mode or eval mode with a valid audio clip

        if not os.path.exists(os.path.join(cache_folder, speaker_id)):
            os.makedirs(os.path.join(cache_folder, speaker_id))

        np.savez(
            os.path.join(cache_folder, speaker_id, f"{speaker_id}_norm_stat.npz"),
            mean=mel_mean,
            std=mel_std,
        )

        save_pickle(
            variable=mel_normalized,
            fileName=os.path.join(
                cache_folder, speaker_id, f"{speaker_id}_normalized.pickle"
            ),
        )
        ...
        print(
            f"Total duration of dataset for {speaker_id} is {info['duration']} seconds"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_directory",
        type=str,
        default=DATA_DIRECTORY_DEFAULT,
        help="Directory holding dataset.",
    )
    parser.add_argument(
        "--preprocessed_data_directory",
        type=str,
        default=PREPROCESSED_DATA_DIRECTORY_DEFAULT,
