        ...
        for i in range(start, end):
            ...
            if (
                librosa.get_duration(
                    filename=os.path.join(root_dir, class_ids[0], fileA)
                )
                < 1
            ...
            duration = librosa.get_duration(
                filename=os.path.join(data_cache, class_ids[0], phase, file)
            )
...
def fetch_with_codec(
    ...
    for file, phase in zip(clips, phase_labels):
        if (
            librosa.get_duration(filename=os.path.join(clean_path, file))
            + duration_saved[phase]
            < duration_max[phase]
            and librosa.get_duration(filename=os.path.join(clean_path, file)) > 1
        ):
            shutil.copyfile(
            ...
            duration_saved[phase] += librosa.get_duration(
                filename=os.path.join(data_cache, "clean", phase, file)
            )
            total_clips[phase] += 1
...
def additive_noise(
    ...
    for clip in train_clips:
        if (
            librosa.get_duration(filename=os.path.join(clean_path, clip))
            + train_duration_saved
            < train_duration_max
            and librosa.get_duration(filename=os.path.join(clean_path, clip)) > 1
        ):
            shutil.copyfile(
            ...
            train_duration_saved += librosa.get_duration(
                filename=os.path.join(clean_path, clip)
            )
...
def rats_noise(
    ...
    for clip in train_clips:
        ...
        if (
            librosa.get_duration(filename=os.path.join(root_dir, "clean", clip))
            + train_duration_saved
            < train_duration_max
            and librosa.get_duration(filename=os.path.join(root_dir, "clean", clip)) > 1
        ):
            shutil.copyfile(
                os.path.join(clean_path, clip),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="transfer_mode",
        type=str,
        choices=["rats", "spectrogram", "npy", "codec", "additive_noise"],
