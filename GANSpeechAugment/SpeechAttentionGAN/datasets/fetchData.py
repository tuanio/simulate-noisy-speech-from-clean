...
def mel(wavspath, sr):
    ...
    for file in wav_files:
        ...
        if spec.shape[-1] >= 64:  # training sample consists of 64 frames
            ...
            info["duration"] += librosa.get_duration(path=wavpath)
...
def preprocess_dataset_spectrogram(
    ...
    for phase, (start, end) in zip(
        ...
        [
            ...
            (train_split + val_split, len(mel_list)),
        ],
    ):
        if phase == "train":
            ## Get mean and norm
            ...
            np.savez(
                os.path.join(cache_folder, class_id, "meta", f"{class_id}_stat"),
                mean=mel_mean,
                std=mel_std,
            )
        ...
        for i in range(start, end):
            ...
            filename = (
                filename[:32] + ".pickle"
            )  ## THIS STEP IS SPECIFIC TO THE CURRENT DATASET TO ENSURE A AND B HAVE SAME FILENAMES

            ##Padding the image
            freq_len, time_len = img.shape
            top_pad = (
                size_multiple - freq_len % size_multiple
                if freq_len % size_multiple != 0
                else 0
            )
            right_pad = (
                size_multiple - time_len % size_multiple
                if time_len % size_multiple != 0
                else 0
            )
            ...
            save_pickle(
                variable=img_padded,
                fileName=os.path.join(cache_folder, class_id, phase, filename),
            )

    save_pickle(
        variable=padding,
        fileName=os.path.join(
            cache_folder, class_id, "meta", f"{class_id}_padding.pickle"
        ),
    )
...
def transfer_aligned_audio_raw(
    root_dir,
    class_ids,
    data_cache,
    train_percent,
    ...
    files_list = [
        x for x in os.listdir(os.path.join(root_dir, class_ids[0])) if x[-4:] == ".wav"
    ]
    ...
    for phase, (start, end) in zip(
        ["train", "test"], [(0, train_split), (num_files - test_split, num_files)]
    ):
        ...
        for i in range(start, end):
            ...
            if (
                librosa.get_duration(
                    path=os.path.join(root_dir, class_ids[0], fileA)
                )
                < 1
            ):  # Skipping very short files
                continue
            if use_genders != "None" and phase != "test":
                if annotations[file] not in use_genders:
                    continue
            shutil.copyfile(
                os.path.join(root_dir, class_ids[0], fileA),
                os.path.join(data_cache, class_ids[0], phase, file),
            )
            shutil.copyfile(
                os.path.join(root_dir, class_ids[1], fileB),
                os.path.join(data_cache, class_ids[1], phase, file),
            )
            duration = librosa.get_duration(
                path=os.path.join(data_cache, class_ids[0], phase, file)
            )
        ...
        print(
            f"{total_duration} seconds ({total_clips} clips) of Audio saved to {phase}."
        )
        if use_genders != "None":
            print(
                f"{male_duration} seconds ({male_clips} clips) of male Audio in {phase}."
            )
            print(
                f"{female_duration} seconds ({female_clips} clips) of female Audio in {phase}."
            )
...
def fetch_from_npy(train_path, test_path, data_cache, sr):
    ...
    for i in range(train_set.shape[0]):
        sf.write(
            os.path.join(data_cache, "clean", "train", f"{i}_audio.wav"),
            train_set[i, :, 0],
            sr,
        )
        sf.write(
            os.path.join(data_cache, "noisy", "train", f"{i}_audio.wav"),
            train_set[i, :, 1],
            sr,
        )
    ...
    for i in range(test_set.shape[0]):
        sf.write(
            os.path.join(data_cache, "clean", "test", f"{i}_audio.wav"),
            test_set[i, :, 0],
            sr,
        )
        sf.write(
            os.path.join(data_cache, "noisy", "test", f"{i}_audio.wav"),
            test_set[i, :, 1],
            sr,
        )


def fetch_with_codec(
    clean_path,
    codec,
    data_cache,
    train_speakers,
    ...
    for file, phase in zip(clips, phase_labels):
        if (
            librosa.get_duration(path=os.path.join(clean_path, file))
            + duration_saved[phase]
            < duration_max[phase]
            and librosa.get_duration(path=os.path.join(clean_path, file)) > 1
        ):
            shutil.copyfile(
                os.path.join(clean_path, file),
                os.path.join(data_cache, "clean", phase, file),
            )
            clean_data, clean_sr = librosa.load(os.path.join(clean_path, file), sr=None)

            if codec == "g726":
                file_orig = os.path.join(data_cache, "clean", phase, file)
                file_8k = os.path.join(
                    data_cache, "noisy", phase, file[:-4] + "_8k.wav"
                )
                file_codec = os.path.join(
                    data_cache, "noisy", phase, file[:-4] + "_fmt.wav"
                )
                file_out = os.path.join(data_cache, "noisy", phase, file)
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_orig} -ar 8k -y {file_8k}"
                )
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_8k} -acodec g726 -b:a 16k {file_codec}"
                )
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_codec} -ar 8k -y {file_out}"
                )
                os.remove(file_8k)
                ...
                file_orig = os.path.join(data_cache, "clean", phase, file)
                file_codec = os.path.join(
                    data_cache, "noisy", phase, file[:-4] + "_fmt.ogg"
                )
                file_out = os.path.join(data_cache, "noisy", phase, file)
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_orig} -c:a libopus -b:a 4.5k -ar 8000 {file_codec}"
                )
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_codec} -ar 8000 {file_out}"
                )
                os.remove(file_codec)
            elif codec == "g723_1":
                file_orig = os.path.join(data_cache, "clean", phase, file)
                ...
                file_out = os.path.join(data_cache, "noisy", phase, file)
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_orig} -acodec g723_1 -b:a 6.3k -ar 8000 {file_codec}"
                )
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_codec} -ar 8000 {file_out}"
                )
                ...
                file_orig = os.path.join(data_cache, "clean", phase, file)
                file_codec = os.path.join(
                    data_cache, "noisy", phase, file[:-4] + "_fmt.gsm"
                )
                file_out = os.path.join(data_cache, "noisy", phase, file)
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_orig} -acodec libgsm -b:a 13k -ar 8000 {file_codec}"
                )
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_codec} -ar 8000 {file_out}"
                )
                ...
                file_raw_input = os.path.join(
                    data_cache, "noisy", phase, file[:-4] + "_in.raw"
                )
                file_enc = os.path.join(
                    data_cache, "noisy", phase, file[:-4] + "_enc.bit"
                )
                file_raw_out = os.path.join(
                    data_cache, "noisy", phase, file[:-4] + "_out.raw"
                )
                file_out = os.path.join(data_cache, "noisy", phase, file)
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_orig} -ar 8k -y {file_8k}"
                ...
                run(
                    f"ffmpeg -i {file_8k} -f s16le -acodec pcm_s16le {file_raw_input}"
                )  # Convert to raw
                run(
                    f'codec2/build_linux/src/c2enc {defaults["codec2_bitrate"]} {file_raw_input} {file_enc}'
                )  # Encode
                run(
                    f'codec2/build_linux/src/c2dec {defaults["codec2_bitrate"]} {file_enc} {file_raw_out}'
                )  # Decode
                run(
                    f"ffmpeg -f s16le -ar 8k -ac 1 -i {file_raw_out} {file_out}"
                )  # Convert to wav
            ...
            duration_saved[phase] += librosa.get_duration(
                path=os.path.join(data_cache, "clean", phase, file)
            )
            total_clips[phase] += 1

    print(
        f"Saved {duration_saved['train']} seconds and {total_clips['train']} clips of audio to train."
    )
    print(
        f"Saved {duration_saved['test']} seconds and {total_clips['test']} clips of audio to test."
    )
...
def additive_noise(
    ...
    for clip in train_clips:
        if (
            librosa.get_duration(path=os.path.join(clean_path, clip))
            + train_duration_saved
            < train_duration_max
            and librosa.get_duration(path=os.path.join(clean_path, clip)) > 1
        ):
            shutil.copyfile(
                os.path.join(clean_path, clip),
                os.path.join(data_cache, "clean", "train", clip),
            )
            ...
            result = (
                defaults["clean_additive_weight"] * clean_data
                + defaults["noisy_additive_weight"]
                * noise_train[start : start + clean_data.shape[0]]
            )
            sf.write(os.path.join(data_cache, "noisy", "train", clip), result, clean_sr)
            train_duration_saved += librosa.get_duration(
                path=os.path.join(clean_path, clip)
            )
    ...
    for clip in test_clips:
        if (
            ...
            shutil.copyfile(
                os.path.join(clean_path, clip),
                os.path.join(data_cache, "clean", "test", clip),
            )
            clean_data, clean_sr = librosa.load(os.path.join(clean_path, clip), sr=None)
            assert clean_sr == noise_sr
            ...
            result = (
                0.5 * clean_data + 0.5 * noise_test[start : start + clean_data.shape[0]]
            )
            sf.write(os.path.join(data_cache, "noisy", "test", clip), result, clean_sr)
            test_duration_saved += librosa.get_duration(
                path=os.path.join(clean_path, clip)
            )
...
def rats_noise(
    ...
    for clip in train_clips:
        ...
        if (
            librosa.get_duration(path=os.path.join(root_dir, "clean", clip))
            + train_duration_saved
            < train_duration_max
            and librosa.get_duration(path=os.path.join(root_dir, "clean", clip)) > 1
        ):
            shutil.copyfile(
                os.path.join(clean_path, clip),
                os.path.join(data_cache, "clean", "train", renamed_clip),
            )
            shutil.copyfile(
                os.path.join(noisy_path, noisy_clip),
                os.path.join(data_cache, "noisy", "train", renamed_clip),
            )
            train_duration_saved += librosa.get_duration(
                path=os.path.join(clean_path, clip)
            )
    ...
    for clip in test_clips:
        ...
        if (
            ...
            shutil.copyfile(
                os.path.join(clean_path, clip),
                os.path.join(data_cache, "clean", "test", renamed_clip),
            )
            shutil.copyfile(
                os.path.join(noisy_path, noisy_clip),
                os.path.join(data_cache, "noisy", "test", renamed_clip),
            )
            test_duration_saved += librosa.get_duration(
                path=os.path.join(clean_path, clip)
            )
...
def transfer_timit(
    ...
    train_speakers_noisy = list(
        set(noisy_speakers) - set(test_speakers) - set(val_speakers)
    )
    ...
    clean_clips_train_all = glob.glob(
        os.path.join(clean_train_path, "**/*.wav"), recursive=True
    )
    noisy_clips_train_all = []
    for speaker in train_speakers_noisy:
        noisy_clips_train_all.extend(
            glob.glob(os.path.join(noisy_path, speaker, "*.wav"))
        )
    ...
    for clip in clean_clips_train_all:
        ...
        if (
            librosa.get_duration(path=clip) + clean_train_duration_saved
            < train_duration_max
            and librosa.get_duration(path=clip) > 1
        ):
            shutil.copyfile(clip, os.path.join(data_cache, "clean", "train", basename))
            clean_train_duration_saved += librosa.get_duration(path=clip)
    ...
    for clip in noisy_clips_train_all:
        basename = os.path.basename(clip)
        if (
            librosa.get_duration(path=clip) + noisy_train_duration_saved
            < train_duration_max
            and librosa.get_duration(path=clip) > 1
        ):
            shutil.copyfile(clip, os.path.join(data_cache, "noisy", "train", basename))
            noisy_train_duration_saved += librosa.get_duration(path=clip)
    ...
    for clip in val_clips_all:
        ...
        if (
            librosa.get_duration(
                path=os.path.join(timit_dir, "test", "clean", clip)
            )
            + val_duration_saved
            < val_duration_max
            and librosa.get_duration(
                path=os.path.join(timit_dir, "test", "clean", clip)
            )
            > 1
        ):
            shutil.copyfile(
                os.path.join(timit_dir, "test", "clean", clip),
                os.path.join(data_cache, "clean", "val", basename),
            )
            shutil.copyfile(
                os.path.join(noisy_path, clip),
                os.path.join(data_cache, "noisy", "val", basename),
            )
            val_duration_saved += librosa.get_duration(
                path=os.path.join(timit_dir, "test", "clean", clip)
            )
    ...
    for clip in test_clips_all:
        ...
        if (
            ...
            shutil.copyfile(
                os.path.join(timit_dir, "test", "clean", clip),
                os.path.join(data_cache, "clean", "test", basename),
            )
            shutil.copyfile(
                os.path.join(noisy_path, clip),
                os.path.join(data_cache, "noisy", "test", basename),
            )
            test_duration_saved += librosa.get_duration(
                path=os.path.join(timit_dir, "test", "clean", clip)
            )
...
def transfer_timit_parallel(
    ...
    for clip in train_clips_all:
        ...
        if (
            ...
            shutil.copyfile(
                os.path.join(timit_dir, "test", "clean", clip),
                os.path.join(data_cache, "clean", "train", basename),
            )
            shutil.copyfile(
                os.path.join(noisy_path, clip),
                os.path.join(data_cache, "noisy", "train", basename),
            )
            train_duration_saved += librosa.get_duration(
                path=os.path.join(timit_dir, "test", "clean", clip)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="audio_path",
        type=str,
        default=defaults["audio_data_path"],
        help="Path to audio root folder",
    )
    parser.add_argument(
        "--timit-dir",
        dest="timit_dir",
        type=str,
        default=defaults["timit_dir"],
        help="Path to timit dir",
    )
    parser.add_argument(
        "--timit_noise_type",
        dest="timit_noise_type",
        type=str,
        default=defaults["timit_noise_type"],
        help="timit noise type: helicopter, cabin",
    )
    parser.add_argument(
        "--source_sub_directories",
        dest="sub_directories",
        type=str,
        default=defaults["source_sub_directories"],
        help="Sub directories for data",
    )
    parser.add_argument(
        "--data_cache",
        dest="data_cache",
        type=str,
        default=defaults["data_cache"],
        help="Directory to Store data and meta data.",
    )
    parser.add_argument(
        "--annotations_path",
        ...
        help="Path to CSV containing gender annotations.Use only if --use_genders is not None.Ignored for --transfer_mode [spectrogram|npy]",
    )
    parser.add_argument(
        "--train_percent",
        dest="train_percent",
        type=int,
        default=defaults["train_percent"],
        help="Percentage for train split.Ignored for --transfer_mode npy or additive_noise.",
    )
    parser.add_argument(
        "--test_percent",
        dest="test_percent",
        type=int,
        default=defaults["test_percent"],
        help="Percentage for test split.Ignored for --transfer_mode npy or additive_noise.",
    )
    parser.add_argument(
        "--size_multiple",
        dest="size_multiple",
        type=int,
        default=defaults["size_multiple"],
        help="Required Factor of Dimensions ONLY if spectrogram mode of tranfer is used",
    )
    parser.add_argument(
        "--sampling_rate",
        dest="sampling_rate",
        ...
        help="Sampling rate for audio. Use if tranfer_mode is spectrogram or npy",
    )
    parser.add_argument(
        "--transfer_mode",
        dest="transfer_mode",
        ...
        help="Transfer files as raw audio ,converted spectrogram, from npy files, using codecor adding noise.",
    )
    parser.add_argument(
        "--use_genders",
        dest="use_genders",
        type=str,
        default=defaults["use_genders"],
        help="Genders to include in train set. Pass None if you do not want to check genders.Ignored for --transfer_mode [spectrogram|npy]",
    )
    parser.add_argument(
        "--npy_train_source",
        dest="npy_train_source",
        type=str,
        default=defaults["npy_train"],
        help="Path where npy train set is present.",
    )
    parser.add_argument(
        "--npy_test_source",
        dest="npy_test_source",
        type=str,
        default=defaults["npy_test"],
        help="Path where npy test set is present.",
    )
    parser.add_argument(
        "--clean_path",
        dest="clean_path",
        type=str,
        default=defaults["clean_path"],
        help="Path to clean audio files. Only use if --transfer_mode is codec or additive_noise.",
    )
    parser.add_argument(
        "--codec_name",
        dest="codec_name",
        type=str,
        default=defaults["codec_name"],
        choices=["g726", "ogg", "g723_1", "gsm", "codec2"],
        help="Name of codec to be used. Only use if --transfer_mode is codec.",
    )
    parser.add_argument(
        "--noise_file",
        dest="noise_file",
        type=str,
        default=defaults["noise_file"],
        help="Path to file containing noise.",
    )
    parser.add_argument(
        "--train_speakers",
        dest="train_speakers",
        nargs="+",
        type=str,
        default=defaults["train_speakers"],
        help="Ids of speakers to be used in train set. Use only if --transfer_mode is additive_noise.",
    )
    parser.add_argument(
        "--test_speakers",
        dest="test_speakers",
        nargs="+",
        type=str,
        default=defaults["test_speakers"],
        help="Ids of speakers to be used in test set. Use only if --transfer_mode is additive_noise.",
    )
    parser.add_argument(
        "--train_duration_max",
        dest="train_duration_max",
        type=int,
        default=defaults["train_duration_max"],
        help="Max duration of train dataset. Use only if --transfer_mode is additive_noise.",
    )
    parser.add_argument(
        "--val_duration_max",
        dest="val_duration_max",
        type=int,
        default=defaults["val_duration_max"],
        help="Max duration of val dataset. Use only if --transfer_mode is additive_noise.",
    )
    parser.add_argument(
        "--test_duration_max",
        dest="test_duration_max",
        type=int,
        default=defaults["test_duration_max"],
        help="Max duration of test dataset. Use only if --transfer_mode is additive_noise.",
    )
    parser.add_argument(
        "--noise_dB",
        dest="noise_dB",
        type=int,
        default=defaults["timit_noise_dB"],
        help="SNR for TIMIT noise dataset. Use only if --transfer_mode is timit.",
    )
    ...
    if args.transfer_mode == "spectrogram":
        for class_id in args.sub_directories:
            preprocess_dataset_spectrogram(
                os.path.join(args.audio_path, class_id),
                class_id,
                args.sampling_rate,
                args.data_cache,
                args.train_percent,
        ...
        rats_noise(
            ...
            args.test_duration_max,
        )
    elif args.transfer_mode == "npy":
        fetch_from_npy(
            args.npy_train_source,
            args.npy_test_source,
            args.data_cache,
            args.sampling_rate,
        )
        ...
        transfer_timit(
            ...
            args.test_duration_max,
            args.timit_noise_type,
            args.noise_dB,
            defaults["val_required"],
        )
    elif args.transfer_mode == "timit_parallel":
        transfer_timit_parallel(
            defaults["timit_dir"],
            args.data_cache,
            defaults["timit_test_speakers"],
            args.train_duration_max,
            args.test_duration_max,
            defaults["timit_noise_type"],
            args.noise_dB,
        )
