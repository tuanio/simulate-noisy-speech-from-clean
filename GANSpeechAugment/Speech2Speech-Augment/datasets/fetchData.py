...
AUDIO_DATA_PATH_DEFAULT = "/data/tuanio/data/noise/TIMIT_DA_target_helicopter"
SUBDIRECTORIES_DEFAULT = ["clean", "noisy"]
CACHE_DEFAULT = "/data/tuanio/data/cache/Speech2Speech/data_cache"
SAMPLING_RATE = 8000
...
def preprocess_dataset_spectrogram(data_path, class_id, args):
    ...
    for phase, (start, end) in zip(
        ...
        for i in range(start, end):
            ...
            top_pad = (
                args.size_multiple - freq_len % args.size_multiple
                if freq_len % args.size_multiple != 0
                else 0
            )
            right_pad = (
                args.size_multiple - time_len % args.size_multiple
                if time_len % args.size_multiple != 0
                else 0
            )
...
def fetch_with_codec(
    ...
    for phase, (start, end) in zip(
        ...
        for i in range(start, end):
            ...
            if (
                librosa.get_duration(filename=os.path.join(clean_path, fileA)) < 1
            ):  # Skipping very short files
                continue
            if use_genders != "None" and phase != "test":
                if annotations[file] not in use_genders:
            ...
            if codec == "g726":
                ...
                file_temp = os.path.join(
                    data_cache, "noisy", phase, file[:-4] + "_temp.wav"
                )
                run(
                ...
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_codec} -ar 8k -y {file_temp}"
                )
                os.remove(file_8k)
                ...
                run(
                    f"ffmpeg -hide_banner -loglevel error -i {file_codec} -ar 8000 {file_temp}"
                )
                os.remove(file_codec)
            elif codec == "g723_1":
                file_orig = os.path.join(data_cache, "clean", phase, file)
            ...
            sf.write(
                os.path.join(data_cache, "noisy", phase, file), data1, SAMPLING_RATE
            )
            sf.write(
                os.path.join(data_cache, "clean", phase, file), data2, SAMPLING_RATE
            )
            os.remove(file_temp)

            assert librosa.get_duration(
                filename=os.path.join(data_cache, "clean", phase, file)
            ) == librosa.get_duration(
                filename=os.path.join(data_cache, "noisy", phase, file)
            )
            duration = librosa.get_duration(
                filename=os.path.join(data_cache, "clean", phase, file)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="audio_path",
        type=str,
        default=AUDIO_DATA_PATH_DEFAULT,
    ...
    parser.add_argument(
        ...
        type=str,
        default=SUBDIRECTORIES_DEFAULT,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Percentage for train split.Ignored for --transfer_mode npy.",
    )
    parser.add_argument(
        "--test_percent",
        dest="test_percent",
        type=int,
        default=15,
        help="Percentage for test split.Ignored for --transfer_mode npy.",
    )
    parser.add_argument(
        "--size_multiple",
        dest="size_multiple",
    ...
    parser.add_argument(
        ...
        dest="transfer_mode",
        type=str,
        choices=["audio", "spectrogram", "npy", "codec"],
        default="audio",
        help="Transfer files as raw audio ,converted spectrogram, from npy files or using codec.",
    )
    parser.add_argument(
        "--use_genders",
        dest="use_genders",
    ...
    parser.add_argument(
        ...
        type=str,
        default=NPY_TEST_DEFAULT,
        help="Path where npy test set is present.",
    )
    parser.add_argument(
        "--codec_clean_path",
        dest="codec_clean_path",
        type=str,
        default=os.path.join(AUDIO_DATA_PATH_DEFAULT, "clean"),
        help="Path to clean audio files. Only use if --transfer_mode is codec.",
    )
    parser.add_argument(
        "--codec_name",
        dest="codec_name",
    ...
    if args.transfer_mode == "spectrogram":
        for class_id in args.sub_directories:
            preprocess_dataset_spectrogram(
                os.path.join(args.audio_path, class_id), class_id, args
            )
    elif args.transfer_mode == "audio":
        transfer_aligned_audio_raw(
            args.audio_path,
            args.sub_directories,
            args.data_cache,
            args.train_percent,
            args.test_percent,
            args.use_genders,
