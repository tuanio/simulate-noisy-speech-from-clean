...
def save_results_as_audio_and_spec(real_a, real_b, fake_b, image_path, save_dir):
    ...
    padding = load_pickle_file(
        os.path.join(CACHE_DEFAULT, "noisy", "meta", "noisy_padding.pickle")
    )
    clean_stats = np.load(
        os.path.join(CACHE_DEFAULT, "clean", "meta", "clean_stat.npz")
    )
    noisy_stats = np.load(
        os.path.join(CACHE_DEFAULT, "noisy", "meta", "noisy_stat.npz")
    )
    ...
    for img, suff in zip(imgs, suffs):
        ...
        save_pickle(
            spec, os.path.join(save_dir, "images", image_path[:32] + suff + ".pickle")
        )
        spec = torch.tensor(spec, dtype=torch.float32)
        rev = vocoder.inverse(spec.unsqueeze(0)).cpu().detach()
        torchaudio.save(
            os.path.join(save_dir, "audio", image_path[:32] + suff + ".wav"),
            rev,
...
def get_audio(data_root, padding_path, save_path):
    ...
    for file in os.listdir(data_root):
        ...
        torchaudio.save(
            os.path.join(save_path, file[:-4] + ".wav"),
            rev,
            sample_rate=SAMPLING_RATE,
            bits_per_sample=16,
        )
