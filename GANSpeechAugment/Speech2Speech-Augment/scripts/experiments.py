...
def validate(name, epochs, data_cache, results_dir):
    ...
    for epoch in epochs:
        run(
            f"python test.py --dataroot data_cache  --split val --name {name} --model pix2pix --direction AtoB --netG resnet_9blocks  --input_nc 1 --output_nc 1 --dataset_mode audio --checkpoints_dir /content/drive/MyDrive/APSIPA/Results/checkpoints --save_dir {results_dir} --epoch {epoch}"
        )
        avg_lsd, std_lsd = lsd(
            os.path.join(data_cache, "noisy", "val"),
            os.path.join(results_dir, name, f"val_{epoch}", "audios", "fake_B"),
            use_gender=False,
        )
...
def apsipa_exp(
    ...
    csv_path,
    sources,
    data_cache="/content/Pix2Pix-VC/data_cache",
    results_dir="/content/Pix2Pix-VC/results",
    ...
    for name, source in zip(names, sources):
        ...
        shutil.copytree(
            os.path.join("/content/drive/MyDrive/APSIPA/Data_Sources", source),
            data_cache,
        )
        ...
        for metric in ["min_lsd_epoch", "min_mssl_epoch"]:
            epoch = info[metric]
            run(
                f"python test.py --dataroot data_cache --name {name} --model pix2pix --direction AtoB --netG resnet_9blocks  --input_nc 1 --output_nc 1 --dataset_mode audio --checkpoints_dir /content/drive/MyDrive/APSIPA/Results/checkpoints --save_dir {results_dir} --epoch {epoch}"
            )
            info[f"avg_test_lsd_{metric}"], info[f"std_test_lsd_{metric}"] = lsd(
                os.path.join(data_cache, "noisy", f"test_{epoch}", "test"),
                os.path.join(results_dir, name, f"test_{epoch}", "audios", "fake_B"),
                use_gender=False,
            )
            info[f"avg_test_mssl_{metric}"], info[f"std_test_mssl_{metric}"] = mssl(
                os.path.join(data_cache, "noisy", f"test_{epoch}", "test"),
                os.path.join(results_dir, name, f"test_{epoch}", "audios", "fake_B"),
                use_gender=False,
            )
            ...
            if info["min_mssl_epoch"] == info["min_lsd_epoch"]:
                ...
                info["avg_test_mssl_min_mssl_epoch"] = info[
                    "avg_test_mssl_min_lsd_epoch"
                ]
                info["std_test_mssl_min_mssl_epoch"] = info[
                    "std_test_mssl_min_lsd_epoch"
                ]
                break
        ...
        info[
            "comment"
        ] = f"Pix2Pix trained for 200 epochs+ 200 with decay on {source} dataset, with Lambda_L1 200."
        log(csv_path, info)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(csv_path):
        cols = [
            ...
            "min_lsd_epoch",
            "min_mssl_epoch",
            "avg_test_lsd_min_lsd_epoch",
            "avg_test_mssl_min_lsd_epoch",
            "avg_test_lsd_min_mssl_epoch",
            "avg_test_mssl_min_mssl_epoch",
            "min_val_lsd",
            "min_val_mssl",
            "std_test_lsd_min_lsd_epoch",
            "std_test_mssl_min_lsd_epoch",
            "std_test_lsd_min_mssl_epoch",
            "std_test_mssl_min_mssl_epoch",
            "avg_val_lsd",
            "avg_val_mssl",
            "std_val_lsd",
            "std_val_mssl",
